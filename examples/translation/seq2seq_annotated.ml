...
*)
module Enc = struct
  type t (hidden_size) = (* ANNOT (record definition): 5 *)
    { forward :  { v:tensor | prod v.shape = 1 }
              -> ~hidden:tensor([1; hidden_size])
              -> tensor([1; hidden_size]) * tensor([1; hidden_size])
    ; zero_state : tensor([1; hidden_size])
    }
...
module D = struct
  type t (hidden_size, output_size) = (* ANNOT (record definition): 6 *)
    { forward :  input_:{ v:tensor | prod v.shape = 1 } -> ~hidden:tensor([1; hidden_size])
              -> ~enc_outputs:{ v:tensor | len v.shape = 3 && nth 0 v.shape = 1 && nth 2 v.shape = hidden_size }
              -> ~is_training:bool -> tensor([1; hidden_size]) * tensor([1; output_size])
    ; zero_state : tensor([1; hidden_size])
    }
  ...
  let create vs ~hidden_size ~output_size =
    ...
    let forward input_ ~hidden ~enc_outputs ~is_training =
      ...
      let enc_outputs : tensor([sz1; max_length; sz3]) = (* ANNOT (branch): 1 *)
        if sz2 = max_length
        then enc_outputs
        else
          Tensor.cat_ ~dim:1
...
let predict ~input_ ~enc ~dec:(dec : { v:D.t | v.hidden_size = enc.hidden_size }) (* ANNOT (higher-order argument): 1 *)
  ~dec_start ~dec_eos ~device =
  let enc_final, enc_outputs =
    let rec loop : state:tensor([1; enc.hidden_size]) -> idx:int -> outs:list(tensor([1; enc.hidden_size])) -> tensor([1; enc.hidden_size]) * list (tensor([1; enc.hidden_size])) (* ANNOT (recursion): 4 *)
    = fun state idx outs ->
...
let train_loss ~input_ ~target ~enc ~dec:(dec : { v:D.t | v.hidden_size = enc.hidden_size }) ~dec_start ~dec_eos ~device = (* ANNOT (higher-order argument): 1 *)
  let enc_final, enc_outputs =
    let rec loop : state:tensor([1; enc.hidden_size]) -> idx:int -> outs:list(tensor([1; enc.hidden_size])) -> tensor([1; enc.hidden_size]) * list (tensor([1; enc.hidden_size])) (* ANNOT (recursion): 4 *)
    = fun state idx outs ->
      if idx = List.length input_
  ...
  let rec loop : ~loss:tensor([]) -> ~state:tensor([1; enc.hidden_size]) -> ~prev:{ v:tensor | prod v.shape = 1 } -> ~target:list(int) -> tensor([]) (* ANNOT (recursion): 4 *)
  = fun ~loss ~state ~prev ~target ->
    ...
    | idx :: target ->
      ...
      then loss
        ...
        let prev : { v:tensor | prod v.shape = 1 } = if use_teacher_forcing then target_tensor else output in (* ANNOT (branch): 1 *)
...
module Loss_stats = struct
  ...
  type t = (* ANNOT (record definition): 2 *)
