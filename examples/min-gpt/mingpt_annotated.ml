...
let gpt vs cfg =
  ...
  in
  let blocks =
    let rec loop : i:int -> xs:{ v:tensor | len v.shape = 3 && nth 1 v.shape = block_size && nth 2 v.shape = n_embd } -> ~is_training:bool -> tensor(xs.shape) = (* ANNOT: 2 *)
      fun i ->
...
let sample cfg
  ~gpt:(gpt : x:tensor([nth 0 v.shape; cfg.block_size]) -> ~is_training:bool -> tensor([nth 0 x.shape; cfg.block_size; cfg.vocab_size])) (* ANNOT: 2 *)
  ~dataset ~device =
...
let train vs ~cfg
  ~gpt:(gpt : x:tensor([nth 0 v.shape; cfg.block_size]) -> ~is_training:bool -> tensor([nth 0 x.shape; nth 1 x.shape; cfg.vocab_size])) ~dataset = (* ANNOT: 2 *)
