  ...
  Checkpointing.loop
    ...
    (fun ~index:batch_idx ->
      ...
      let discriminator_loss, real_loss_d, fake_loss_d =
        ...
        (* EDIT: Replace no_grad with no_grad_t *)
        let gen_z = Tensor.no_grad_t (fun () -> z_dist () |> Layer.forward generator) in
        ...
        let loss_d = Tensor.(real_loss_d - (Tensor.to_device ~device k * fake_loss_d)) in
      ...
      Tensor.fill_float k (Float.max 0. (Float.min 1. (Tensor.float_value k +. (lambda_k *. balance))));
      if batch_idx % 100 = 0
      then
        Stdio.printf
      ...
      then
        Tensor.no_grad_t (fun () -> Layer.forward generator z_test)
