...
let create_generator vs =
  let tr2d ~stride ~padding ~input_dim n =
    Layer.conv_transpose2d_
      ...
      ~input_dim
      n
      (* EDIT: Remove record here, since record types in variants are not supported yet *)
...
let () =
  ...
  let images : { v:tensor | len v.shape = 4 && nth 1 v.shape = 3 && nth 2 v.shape = 64 && nth 3 v.shape = 64 } = Serialize.load ~filename:Sys.argv.(1) in (* ANNOT: 1 *)
