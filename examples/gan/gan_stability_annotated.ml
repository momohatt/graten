...
let resnet_block vs ~input_dim output_dim =
  ...
  let shortcut
    ...
    -> tensor([nth 0 x.shape; output_dim; nth 2 x.shape; nth 3 x.shape]) = (* ANNOT: 2 *)
  ...
  Checkpointing.loop
    ...
    (fun ~index:batch_idx ->
      ...
      let discriminator_loss =
        ...
        let x_fake = Tensor.no_grad_t (fun () -> z_dist () |> generator) in
      ...
      then
        (* EDIT: Replace no_grad with no_grad_t *)
        Tensor.no_grad_t (fun () -> generator z_test)
