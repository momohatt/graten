...
let () =
  ...
  let env = Env_gym_pyml.create "PongDeterministic-v4" ~action_repeat:1 in
  ...
  for episode_idx = 1 to total_episodes do
    ...
    let rec loop state acc_reward =
      ...
      let { Env_gym_pyml.obs = next_state; reward; is_done } = Env_gym_pyml.step env ~action:(2 + action) in
      if render
      then
    ...
    in
    let reward = loop (Env_gym_pyml.reset env |> preprocess) 0. in
    Stdio.printf
      "%d %f (%d/%d frames)\n%!"
      episode_idx
      reward
