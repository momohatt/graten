...
let run t ~model:(model : x:tensor([t.num_procs; t.num_stack; 84; 84]) -> { v:actor_critic | v.num_procs = t.num_procs }) = (* ANNOT: 2 *)
  ...
  for s = 0 to t.num_steps - 1 do
    ...
    in
    let { E.obs = (obs : tensor([t.num_procs; 1; 84; 84])) (* ANNOT: 3 *)
        ; reward = (reward : tensor([t.num_procs]))
        ; is_done = (is_done : tensor([t.num_procs])) } =
