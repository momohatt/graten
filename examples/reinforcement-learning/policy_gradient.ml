...
let () =
  ...
  let env = Env_gym_pyml.create env_name ~action_repeat:1 in
    ...
    let rec loop obs =
      let action =
        Tensor.no_grad (fun () ->
            ...
            |> Tensor.to_int0_exn)
      in
      let { Env_gym_pyml.obs = next_obs; reward; is_done } = Env_gym_pyml.step env ~action in
      Acc.update acc ~obs ~action ~reward;
      if is_done
      then (
        let obs = Env_gym_pyml.reset env in
        Acc.is_done acc;
        if Acc.length acc <= batch_size then loop obs)
      else loop next_obs
    in
    loop (Env_gym_pyml.reset env);
