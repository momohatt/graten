...
let report predictions ~image ~width ~height =
    ...
    List.init
      ...
      ~f:(fun index ->
        ...
        then (
          ...
          (* EDIT: Fix order of fields *)
          then Some { xmin; ymin; xmax; ymax; confidence; class_index; class_confidence }
          else None)
        else None)
    |> List.filter_opt
  in
  ...
  let resize_and_clamp v ~initial_max ~max:m =
...
let () =
  ...
  let image : tensor([1; 3; 224; 224]) = Image.load_image Sys.argv.(2) |> Or_error.ok_exn in (* ANNOT: 1 *)
