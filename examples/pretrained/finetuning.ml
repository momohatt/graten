...
let () =
  ...
  (* EDIT: use List.length_string instead of List.length *)
  let fc1 = Layer.linear train_vs ~input_dim:512 (List.length_string classes) in
  ...
  for epoch_idx = 1 to 20 do
    (* EDIT: Replace a ref float with a 0-dimensional tensor *)
    ...
    Stdio.printf
      ...
      epoch_idx
      (Tensor.float_value sum_loss /. Float.of_int (Dataset_helper.batches_per_epoch dataset ~batch_size))
      (100. *. test_accuracy)
  done
