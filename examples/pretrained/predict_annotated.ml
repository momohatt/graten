...
let () =
  ...
  let image : tensor([1; 3; 224; 224]) = Imagenet.load_image Sys.argv.(2) in (* ANNOT: 1 *)
  let vs = Var_store.create ~name:"rn" ~device:Cpu () in
  let model : x:{ v:tensor | len v.shape = 4 && nth 1 v.shape = 3 } -> ~is_training:bool -> tensor([nth 0 x.shape; 1000]) = (* ANNOT: 2 *)
