...
let downsample vs ~stride ~input_dim output_dim
  : x:{ v:tensor | len v.shape = 4 && nth 1 v.shape = input_dim } -> ~is_training:bool
...
let resnet num_classes vs ~block:_ c1 c2 c3 c4 =
  ...
  in
  let rec make_layer
    : Torch.Var_store.t -> ~stride:int -> ~cnt:int -> ~input_dim:int -> output_dim:int
      -> xs:{ v:tensor | len v.shape = 4 && nth 1 v.shape = input_dim } -> ~is_training:bool
      -> tensor([nth 0 xs.shape; output_dim; (nth 2 xs.shape - 1) // stride + 1; (nth 3 xs.shape - 1) // stride + 1])
